...
| Vercel env | `RADAR_TO` unset → the **team/admin** copy of the daily brief doesn't go out; the brief itself does, via the subscriber path, which never reads `RADAR_TO` (6 active subscribers, 5 Aug). Adding recipients is better done in Admin → Subscribers than here — the var needs a dashboard edit **and** a redeploy. **`OPS_ALERT_TO` is now SET** (7 Aug, one address, redeploy confirmed) — before that the daily health push recorded its alert and reached nobody, because `sendOpsAlert` calls `recordAlert()` BEFORE it tries to send, so a channel with no recipients looked quiet rather than broken. Setting it also takes over the forgot-password alert, which reads `OPS_ALERT_TO || RADAR_TO || ADMIN_EMAILS`: reset requests now go to that ONE address instead of fanning out to every bootstrap admin. Widen it to the `ADMIN_EMAILS` list if more than one person should see them |
