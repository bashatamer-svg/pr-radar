...
// NOTE: `URL` below deliberately shadows the global URL constructor for this
// whole module, which is why URL parsing lives in lib/safe-url.js and is
// imported rather than done inline here.
import { safeExternalUrl } from './safe-url.js';
// The live source list, so brokenFeeds() can tell a BROKEN feed from a RETIRED
// one. sources.js imports nothing, so this cannot cycle.
import { ALL_FEEDS } from './sources.js';
...
/** Record one feed's outcome for this run.
 *
 *  SUCCESS is a plain upsert: stamp last_ok_at, clear the error, reset the
 *  streak to 0. One statement, no read, nothing to race on.
 *
 *  FAILURE used to write `fail_streak: 1` — a literal, every single time. So
 *  the column never counted: a feed dead for three weeks read identically to
 *  one that hiccuped once, which made it useless for the only thing it is for
 *  (naming the persistently-dead feeds) while looking like it worked.
 *
 *  It cannot be fixed by reading the current value and writing value+1. The
 *  runs OVERLAP — the 15-minute urgent poll and the 05:00 daily run both fetch
 *  the ten brand feeds — so two writers read the same number and both write the
 *  same increment, losing one. PostgREST cannot express `set x = x + 1` either;
 *  it sends literal values only.
 *
 *  So the increment goes through a Postgres function
 *  (migrations/2026-08-07-pr-feed-failure-streak.sql), which does it as a single
 *  INSERT..ON CONFLICT DO UPDATE — atomic by construction, and it also handles
 *  a feed that has no row yet.
 *
 *  Without that migration the RPC 404s and the fallback below records the error
 *  WITHOUT touching fail_streak. That is deliberately not the old behaviour: a
 *  stale count is honest about being stale, whereas resetting to 1 actively
 *  asserts something false. Either way last_ok_at is left alone, which is what
 *  brokenFeeds() and the Health feed check actually read. */
export async function recordFeedHealth(feedId, ok, error) {
  ...
  try {
    await rest('rpc/pr_bump_feed_failure', {
      method: 'POST',
      body: JSON.stringify({ p_feed_id: feedId, p_error: message }),
      headers: { Prefer: 'return=minimal' },
    });
    return;
  } catch { /* function not applied — fall through */ }

  // Fallback: record WHAT failed, leave the count alone, and never stamp
  // last_ok_at. `fail_streak` is absent from the payload on purpose — with
  // resolution=merge-duplicates PostgREST only updates the columns it is given,
  // so an existing count survives untouched rather than being overwritten with
  // a number we cannot compute correctly here.
  await rest('pr_feed_health', {
    method: 'POST',
    body: JSON.stringify({ feed_id: feedId, last_error: message }),
    headers: { Prefer: 'resolution=merge-duplicates' },
  });
}

/** Feeds with no successful fetch in three days.
 *  Now carries `fail_streak` and `last_ok_at` too: "failed 40 times in a row"
 *  and "has never worked since it was added" are different problems with
 *  different fixes, and the feed id alone distinguishes neither. */
export async function brokenFeeds() {
  const cutoff = new Date(Date.now() - 3 * 864e5).toISOString();
  const rows = await rest(
    `pr_feed_health?select=feed_id,last_error,fail_streak,last_ok_at` +
      `&or=(last_ok_at.is.null,last_ok_at.lt.${cutoff})`
  );
  // A row for a feed that is no longer a SOURCE is not a broken feed, it is a
  // RETIRED one — and nothing ever fetches it again, so its last_ok_at is
  // frozen and it matches this query forever.
  // `site-newsrooms` was retired on 5 Aug and kept naming itself in the daily
  // brief's footer and on Admin → Health with `fail_streak:0` and no error:
  // not failing, simply never attempted. That is how a check goes permanently
  // amber, and a permanently-amber check takes the ones beside it down with it.
  // Filtered HERE rather than at the two call sites, so they cannot drift; and
  // filtered rather than deleted, because the row is honest history and
  // deleting production data is a human's call.
  const live = new Set(ALL_FEEDS.map((f) => f.id));
  return (rows || []).filter((r) => live.has(r.feed_id));
}
...
/** One subscriber by (lowercased) email, or null.

    Exists so a caller can ASK before writing: `addSubscriber` upserts on the
    unique email with every column present, so calling it for an address that is
    already on the list overwrites that row's categories and WhatsApp number
    with the new call's (empty) values. The user-creation flow therefore looks
    the address up first and only adds, or re-activates, never overwrites. */
export async function getSubscriberByEmail(email) {
  const e = String(email || '').trim().toLowerCase();
  if (!e) return null;
  const rows = await rest(`pr_subscribers?select=id,email,name,categories,whatsapp,active&email=eq.${encodeURIComponent(e)}&limit=1`);
...
/** Edit an existing subscriber in place. Only the fields PRESENT in `patch`
    are written, so a caller changing just the phone cannot blank the name by
    omission. '' clears a value to NULL (never stores an empty string), which
    is how the UI removes a WhatsApp number or a category filter. */
export async function updateSubscriber(id, patch = {}) {
  const body = {};
  if ('email' in patch) body.email = String(patch.email).trim().toLowerCase();
  if ('name' in patch) body.name = String(patch.name || '').trim() || null;
  if ('categories' in patch) {
    const c = Array.isArray(patch.categories) ? patch.categories.map((x) => String(x).trim()).filter(Boolean) : [];
    body.categories = c.length ? c : null;
  }
  if ('whatsapp' in patch) {
    const wa = String(patch.whatsapp || '').replace(/[^\d]/g, '');
    body.whatsapp = wa.length >= 8 ? wa : null;
  }
...
export async function setResolvedUrl(id, url) {
  const n = parseInt(id, 10);
  // Last gate before the column that /api/go redirects to. Callers validate
  // too; this one is here because the column is read back and TRUSTED later,
  // so a bad value written once is a bad Location header forever.
  const safe = safeExternalUrl(url);
  if (!Number.isInteger(n) || !safe) return;
  await rest(`pr_items?id=eq.${n}`, {
    method: 'PATCH',
    body: JSON.stringify({ resolved_url: safe.slice(0, 2000) }),
    headers: { Prefer: 'return=minimal' },
  });
}
...
export async function itemsBetween({ fromIso, toIso, cols = 'lean' } = {}) {
  // 'stats' mirrors itemsForStats' select exactly — /api/stats serves the same
  // aggregates whether the window came from `days` or from an explicit range,
  // so both paths must hand the counting code identical rows. 'full' would work
  // and fetch four columns the aggregation never reads.
  const select = cols === 'full'
    ...
    : cols === 'stats'
      ? 'id,brand,sentiment,category,importance,author,source,published_at,seen_at,headline,summary'
...
export async function recentItems({ days = 7, limit = 200, fromIso = null, toIso = null } = {}) {
  // An explicit range is what a Trends deep-link carries. Without it the board
  // would answer a custom-range row with its nearest FIXED window and show a
  // different set of cards than the row counted — the same class of bug as a
  // narrative opening 20 of its 27 stories.
  const ranged = Boolean(fromIso && toIso);
  const since = ranged ? fromIso : new Date(Date.now() - days * 864e5).toISOString();
  const win = ranged
    ? `&or=(and(published_at.gte.${since},published_at.lt.${toIso}),` +
      `and(published_at.is.null,seen_at.gte.${since},seen_at.lt.${toIso}))`
    : `&or=(published_at.gte.${since},and(published_at.is.null,seen_at.gte.${since}))`;
  const rows = await rest(
    `pr_items?select=*&is_relevant=is.true${win}` +
      `&order=importance.desc,published_at.desc.nullslast,seen_at.desc&limit=${limit}`
  );
  ...
  if (Array.isArray(rows) && rows.length === limit) {
    console.warn(`recentItems: board window capped at ${limit} rows for the ${ranged ? `${since}..${toIso}` : `${days}d`} window — older/lower-importance items omitted`);
  }
  return rows;
}
...
export async function listUsers() {
  const cols = 'id,email,role,name,active,invited_by,last_seen_at,created_at';
  const q = (c) => `pr_users?select=${c}&order=role.asc,created_at.desc`;
  // `reset_requested_at` is OPTIONAL — it arrives with
  // migrations/2026-08-06-pr-users-reset-requested-at.sql, which is applied by
  // hand. Selecting a column that does not exist 400s the whole request, so an
  // unapplied migration would take the entire Users tab down rather than just
  // hiding one chip. Ask for it, fall back without it.
  try { return await rest(q(`${cols},reset_requested_at`)); } catch { return rest(q(cols)); }
}

/* ── forgot-password queue (pr_users.reset_requested_at) ──
   There is deliberately NO self-service reset link: that needs an emailed token,
   which is a whole security surface (expiry, single use, leaking via Referer) to
   get wrong for a team of this size. Instead the request reaches the admin two
   ways at once — an email now, and a row in Admin → Requests that stays until it
   is dealt with. Every function here fails soft: without the migration the
   feature is simply absent, never a 500 on the login screen or the admin page. */

/** Flag that someone asked for a password reset from the login screen.
    Returns true when a real, ACTIVE row was flagged.
    Only ever touches an EXISTING row — a request for an unknown address must not
    create an account — and the caller answers the browser identically either
    way, so the login screen cannot be used to discover who has access. */
export async function requestPasswordReset(email) {
  const e = String(email || '').trim().toLowerCase();
  if (!e) return false;
  try {
    const rows = await rest(`pr_users?email=eq.${encodeURIComponent(e)}&active=is.true`, {
      method: 'PATCH',
      body: JSON.stringify({ reset_requested_at: new Date().toISOString() }),
      headers: { Prefer: 'return=representation' },
    });
    return Array.isArray(rows) && rows.length > 0;
  } catch { return false; }   // column not there yet — nothing to record
}

/** Outstanding reset requests, OLDEST FIRST: the person who has been locked out
    longest is the one to serve next. */
export async function pendingResets() {
  try {
    return await rest(`pr_users?select=id,email,name,role,reset_requested_at&reset_requested_at=not.is.null&order=reset_requested_at.asc`);
  } catch { return []; }
}

/** Answer a request without changing the password — it was a mistake, or the
    person was helped another way. */
export async function clearResetRequest(id) {
  try {
    return await rest(`pr_users?id=eq.${Number(id)}`, {
      method: 'PATCH', body: JSON.stringify({ reset_requested_at: null }), headers: { Prefer: 'return=minimal' },
    });
  } catch { return null; }
}

/** Setting a password ANSWERS any outstanding request, so the queue empties
    itself instead of needing a second "mark done" click. Called from every path
    that sets a password (admin reset, self-service change, first signup) —
    PR Radar keeps the password in Supabase Auth, so there is no single DB write
    to hang this on the way an app-local hash would give you. */
export async function clearResetForEmail(email) {
  const e = String(email || '').trim().toLowerCase();
  if (!e) return null;
  try {
    return await rest(`pr_users?email=eq.${encodeURIComponent(e)}`, {
      method: 'PATCH', body: JSON.stringify({ reset_requested_at: null }), headers: { Prefer: 'return=minimal' },
    });
  } catch { return null; }
}
...
/* ── Scheduled-run ledger (pr_runs) ─────────────────────────────────────────
   Fail-soft by contract: see lib/runs.js. These THROW on a missing table and
   the caller swallows it, so an un-migrated database records nothing and the
   pipeline is unaffected. */

/** Open a run row. → the new id, or null if the ledger is unavailable. */
export async function insertRun(row) {
  const res = await rest('pr_runs', {
    method: 'POST',
...
/** Recent runs, newest first. THROWS when the table is absent so the health
    check can say "not recorded" rather than "never ran" — opposite facts. */
export async function recentRuns({ days = 3, limit = 200 } = {}) {
  const since = new Date(Date.now() - days * 864e5).toISOString();
  return rest(
    `pr_runs?select=job,started_at,completed_at,status,duration_ms,stored_count,relevant_count,alert_count,error_summary,git_sha` +
      `&started_at=gte.${since}&order=started_at.desc&limit=${limit}`
  );
}

/** The newest run of each named job, with NO time horizon.
 *
 *  `recentRuns()` answers "what has happened lately", which is the wrong query
 *  for "when did job X last run": once a job has been dead longer than the
 *  window, its last row drops out and the job looks as though it was never
 *  recorded — so the overdue check skipped it and a ten-day outage went quiet
 *  on the page. One indexed `limit=1` read per job (pr_runs_job_started_idx),
 *  so the cost is fixed and tiny however large the table grows. PostgREST has
 *  no DISTINCT ON, and a single ordered query cannot be bounded safely: the
 *  15-minute poll writes 96 rows a day, so any row cap is minutes of urgent
 *  runs and would hide the weekly jobs entirely.
 *
 *  THROWS when the table is absent, exactly like recentRuns(). */
export async function latestRuns(jobs) {
  const rows = await Promise.all((jobs || []).map((job) =>
    rest(`pr_runs?select=job,started_at,completed_at,status,duration_ms,error_summary,git_sha`
      + `&job=eq.${encodeURIComponent(job)}&order=started_at.desc&limit=1`)));
  return rows.flat().filter(Boolean);
}

/** Every migration this database records as applied. THROWS when the ledger
    table is absent, so lib/migrations.js can report "unavailable" rather than
    "everything is missing" — those are opposite facts, and reporting the
    pessimistic one would warn about six migrations that may all be applied.
    READ-ONLY: nothing in the application ever writes here. Applying a migration
    is a human action taken with explicit approval, and an app that could record
    (or perform) its own schema changes is one that changes production while
    nobody is watching. */
export async function appliedMigrations() {
  return rest('pr_schema_migrations?select=id,applied_at,note&order=id.asc&limit=200');
}
