...
export function digestEligibleCount({ hours = 24, minImportance = 2 } = {}) {
  const since = new Date(Date.now() - hours * 36e5).toISOString();
  return countRows(`pr_items?is_relevant=is.true&importance=gte.${minImportance}&seen_at=gte.${since}`);
}
