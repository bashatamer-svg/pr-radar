...
async function aiGroup(items, signalKey) {
  ...
  // The id and the brand are OURS; the headline and the summary are not — the
  // headline came from a feed, and the summary is the classifier's paraphrase
  // OF a feed. Both are neutralised, and the block is fenced.
  const lines = items.map((it) => {
    const sum = neutralize(it.summary, { max: 180 });
    return `${it.id} | ${brandOf(it)} | ${neutralize(it.headline, { max: 160 })}${sum ? `\n   ${sum}` : ''}`;
  }).join('\n');
