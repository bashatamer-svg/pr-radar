...
export async function principal(req) {
  ...
  if (RADAR && (bearer === RADAR || t === RADAR)) return { kind: 'service', actor: 'service:token', role: 'admin' };
  if (CRON && (bearer === CRON || t === CRON)) return { kind: 'service', actor: 'service:cron', role: 'admin' };
  if (bearer) {
