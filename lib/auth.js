...
/* ── Provisioning a person (Admin → Users) ──────────────────────────────────
   ...
   sets a starter password too, and the welcome email carries it.
   ─────────────────────────────────────────────────────────────────────────── */

// Supabase refuses a password under 6 characters, so the convention below is
// topped up in that one case rather than allowed to fail the create. The floor
// is 6, NOT the 8 that /api/auth enforces: that one governs a password a person
// CHOOSES, and raising this to match it would rename the starter password for
// every four-letter first name ("mona123" → "mona1234"), breaking the one thing
// the convention is for — an admin being able to say it without looking it up.
const MIN_PRESET = 6;

/** First name for the starter password: first word of the display name,
    lowercased, letters/digits only; falls back to the email's local part up to
    the first separator. Never empty. */
export function firstNameOf(name, email) {
  const clean = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  let first = clean(String(name || '').trim().split(/\s+/)[0]);
  if (first.length < 2) {
    const local = String(email || '').split('@')[0] || '';
    first = clean(local.split(/[._+-]/)[0] || local);
  }
  return first || 'radar';
}

/** The starter-password convention: first name + "123" ("Tamer Basha" →
    tamer123). A one- or two-letter first name would land under the floor above,
    so the digit run is EXTENDED rather than the name mangled ("Ed" → ed1234) —
    the welcome email states the password verbatim, so it never has to be
    guessable, only sayable over the phone. */
export function presetPasswordFor(name, email) {
  let pw = `${firstNameOf(name, email)}123`;
  for (const d of '456789') { if (pw.length >= MIN_PRESET) break; pw += d; }
  return pw;
}

/** Provision a person in one step: the allowlist row PLUS a starter password,
    so someone added in Admin → Users can sign in immediately.
    → { user, credentials, error }

    `credentials` is:
      'created'  — the starter password was set; it is safe to email it
