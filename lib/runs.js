...
/** A MANUAL run is recorded under its own key.
 *
 *  Every one of these endpoints is `requireOperator`-gated, which admits a
 *  signed-in admin as well as the cron — so an interactive `/api/geo` hit wrote
 *  the same `geo` key Health reads as "the Monday cron fired". One diagnostic
 *  click reset an eight-day overdue clock and hid a dead job for another week,
 *  which is the exact blindness this ledger was added to remove.
 *
 *  A manual run is still WORTH recording: it can stall or throw like any other,
 *  and the completion check reads every row. It simply must not answer the
 *  question "did the schedule fire?", so it goes under `<job>:manual` and the
 *  overdue clock never sees it. */
export const MANUAL_SUFFIX = ':manual';

/** True only for the cron's own service principal. A human holding CRON_SECRET
    is indistinguishable from the cron and is counted as scheduled — that is a
    deliberate limit, not an oversight: the token IS the schedule's identity. */
export const isScheduled = (who) => who?.kind === 'service' && who?.actor === 'service:cron';

/** The ledger key for a run of `job` triggered by `who`. */
export const runJob = (job, who) => (isScheduled(who) ? job : `${job}${MANUAL_SUFFIX}`);
...
const SCHEDULED_JOBS = new Set(Object.keys(OVERDUE_H));

/** Build the run-ledger checks.
 *
 *  `rows` is recentRuns() output — a short window, which is what the failure,
 *  stall and duration questions need. `rows === null` means the table is absent
 *  — reported as UNAVAILABLE, never as "nothing ran". Those are opposite facts
 *  and guessing the alarming one would put the page permanently red on an
 *  un-migrated database.
 *
 *  `latest` is the newest row per job with NO time horizon, and it is what the
 *  overdue clock reads. Deriving "when did job X last run?" from the short
 *  window alone had the outage cure itself: once a job had been dead longer
 *  than the horizon its last row fell out of the query, the job looked as
 *  though it had never been recorded, and the overdue check skipped it — so a
 *  cron stopped for ten days went QUIET on the page instead of critical. */
export function runChecks(rows, now = Date.now(), latest = null) {
  ...
  // `latest` first: it is unbounded, so it holds the row a long outage has
  // already aged out of `rows`. Manual keys are excluded here and only here —
  // they still reach the completion check below, they just cannot answer "did
  // the schedule fire?".
  const seen = new Map();     // scheduled job -> newest row
  for (const r of [...(Array.isArray(latest) ? latest : []), ...rows]) {
    if (r && SCHEDULED_JOBS.has(r.job) && !seen.has(r.job)) seen.set(r.job, r);
  }
  ...
  if (!seen.size) {
    out.push({
      id: 'runs', label: 'Scheduled runs', state: 'unknown',
      detail: `the ledger is present but holds no scheduled run yet${rows.length ? ` (${rows.length} manual run(s) recorded)` : ''}`,
