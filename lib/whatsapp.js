...
/** Who actually gets paged. Admin → Subscribers is the source of truth so the
    list can be changed without a redeploy; WHATSAPP_TO still wins when set,
    which leaves room for a curated crisis list that is not the brief's
    audience. Fail-soft: a DB error falls back to the env var rather than
    silencing the channel. */
export async function resolveWhatsappRecipients() {
  const env = whatsappRecipients();
  if (env.length) return env;
  try { return await whatsappSubscribers(); }
  catch (e) { console.error('whatsapp: subscriber lookup failed', e.message); return []; }
}
...
export async function sendWhatsAppUrgent(item) {
  ...
  const to = await resolveWhatsappRecipients();
