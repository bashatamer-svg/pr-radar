...
import { fetchAuthorProbe, cleanAuthor, authorAiEnabled, resetAuthorAiBudget } from './author.js';
...
/** Fill authors for a set of authorless items. Returns { filled, breakdown } where
    breakdown explains every item's outcome — so a run of 0 fills is diagnosable:
      ...
      fetchFailed — had a real link but every fetch failed (403/blocked/timeout)
      noByline    — fetched a real article OK, but it carried no byline
      writeFailed — byline found but the DB write errored */
export async function fillMissingAuthors(stale) {
  const breakdown = { filled: 0, unresolved: 0, fetchFailed: 0, noByline: 0, writeFailed: 0 };
  if (!stale || !stale.length) return { filled: 0, breakdown };
  resetAuthorAiBudget();   // AI cap is per SWEEP — a warm lambda must not carry exhaustion over
  ...
  const outcomes = await Promise.all(stale.map(async (it) => {
    ...
    // tracking whether ANY fetch actually succeeded (to split blocked vs bylineless).
    let hadFetchable = false, anyFetchOk = false;
    if (!author) {
      ...
      for (const u of urls) {
        const { author: raw, outcome } = await fetchAuthorProbe(u, (insts[0] && insts[0].outlet) || it.source, { headline: it.headline });
        if (outcome !== 'fetch-failed') anyFetchOk = true;
        const person = cleanAuthor(raw, (insts[0] && insts[0].outlet) || it.source);
        if (person) { author = person; break; }
      }
    }
    if (!author) return !hadFetchable ? 'unresolved' : (anyFetchOk ? 'noByline' : 'fetchFailed');
...
export async function sweepAuthors({ days = 3, limit = 40 } = {}) {
  ...
  return {
    ...
    unresolved: breakdown.unresolved, fetchFailed: breakdown.fetchFailed,
    noByline: breakdown.noByline, writeFailed: breakdown.writeFailed,
    aiEnabled: authorAiEnabled(),
  };
}
