...
${UNTRUSTED_DATA_RULES}`;
  ...
  const res = await fetchT('https://api.anthropic.com/v1/messages', {
    ...
    headers: { 'content-type': 'application/json', 'x-api-key': process.env.ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01' },
        // The ANSWER is another AI's output about Vodafone — third-party text by
    // any definition, and the one input here nobody controls. Fenced and
    // neutralised like every other source text; the QUESTION is ours, but it
    // rides the same block so the boundary is unambiguous.
    body: JSON.stringify({ model: EVAL_MODEL, max_tokens: 500, system, messages: [{ role: 'user', content:
      fenceUntrusted(`QUESTION: ${neutralize(question, { max: 300 })}\n\nANSWER:\n${neutralize(answer, { max: 6000 })}`) }] }),
  });
  if (!res.ok) throw new Error(`audit ${res.status}`);
  const data = await res.json();
  recordUsage('geo', EVAL_MODEL, data);   // fire-and-forget spend gauge; never awaited
