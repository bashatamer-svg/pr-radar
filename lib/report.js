...
export async function buildReportRange({ fromMs, toMs }) {
  ...
  // Coverage log — the raw extract, newest first, capped for document sanity.
...
export function renderReport(data, { period = 'week', title = null, rangeText = null, appendix = false } = {}) {
  ...
  if (appendix && Array.isArray(data.appendix)) {
    ...
    appendixHtml = `
      ${sectionHead('Appendix — coverage log', T.OXBLOOD, `${data.appendix.length} stor${data.appendix.length === 1 ? 'y' : 'ies'}, newest first`)}
      ...
      <div style="font-family:${T.FONT};font-size:10.5px;line-height:1.6;color:${T.MUTED};">
        ...
        (brand, tone, reach 1–5) and de-duplicated across outlets; unbylined articles are attributed to the publication's newsroom.
