...
// Shared surge math over an ARBITRARY bucketing of the negatives, so the same
// statistics serve both the calendar-day and the rolling-24h windows. `bucketOf`
// maps an item to a bucket key (or null to drop it); `currentKey` is the bucket
// under test, `baseKeys` the trailing baseline. Zero-filled over `keys` so quiet
// buckets still count in the stddev. Returns the surge objects that clear all
// three bars (floor, above-mean, mean + K·stddev).
function bucketedSurges(negatives, weight, { keys, currentKey, baseKeys, bucketOf, K, floor, windowDays, mode }) {
  const inAxis = new Set(keys);
  const vol = {};
  for (const b of SERIES) vol[b] = Object.fromEntries(keys.map((k) => [k, 0]));
  for (const n of negatives) {
    const k = bucketOf(n);
    if (k == null || !inAxis.has(k)) continue;
    vol[brandOf(n)][k] += weight.get(n.id) || 1;
  }
  const surges = [];
  for (const b of SERIES) {
    const cur = vol[b][currentKey] || 0;
    const base = baseKeys.map((k) => vol[b][k] || 0);
    ...
    // Clears the statistical bar, the noise floor, and is genuinely above the
    // norm (guards the stddev≈0 quiet-brand case).
    if (cur >= floor && cur > mean && cur >= threshold) {
      surges.push({
        brand: b, today: r1(cur), mean: r1(mean), stddev: r1(stddev),
        threshold: r1(threshold), multiple: r1(cur / Math.max(mean, 1)),
        windowDays, mode, topStories: [],
      });
    }
  }
  return surges;
...
export async function detectSurges(opts = {}) {
  ...
  // Two bucketings of the SAME negatives, both fail-soft:
  //   • calendar-day  (default): today-so-far vs prior FULL Cairo days.
  //   • rolling-24h   (Task 6, opt-in): the trailing 24h vs prior 24h windows,
  //     so an early-in-the-day spike isn't diluted by a partial calendar day.
  // The rolling result is computed and LOGGED every run for calibration; it is
  // only promoted to the returned (and thus alertable) source when SURGE_ROLLING=1.

  // calendar-day axis (oldest → today); today is the last key.
  const days = [];
  const seenDay = new Set();
  for (let i = windowDays; i >= 0; i--) {
    const d = cairoDay.format(new Date(Date.now() - i * 864e5));
    if (!seenDay.has(d)) { seenDay.add(d); days.push(d); }
  }
  const today = days[days.length - 1];
  const dayS = bucketedSurges(negatives, weight, {
    keys: days, currentKey: today, baseKeys: days.slice(0, -1),
    bucketOf: dayOf, K, floor, windowDays, mode: 'calendar-day',
  });

  // rolling trailing-24h buckets: index = whole 24h periods since publish
  // (0 = last 24h = "current"; 1..windowDays = the baseline windows).
  const rollKeys = Array.from({ length: windowDays + 1 }, (_, i) => i);
  const rollBucketOf = (it) => {
    const t = it.published_at || it.seen_at;
    if (!t) return null;
    const ms = Date.now() - new Date(t).getTime();
    if (!Number.isFinite(ms) || ms < 0) return null;
    const idx = Math.floor(ms / 864e5);
    return idx <= windowDays ? idx : null;
  };
  let rollS = null;
  try {
    rollS = bucketedSurges(negatives, weight, {
      keys: rollKeys, currentKey: 0, baseKeys: rollKeys.slice(1),
      bucketOf: rollBucketOf, K, floor, windowDays, mode: 'rolling-24h',
    });
  } catch (e) { console.error('surge rolling-24h shadow failed (non-fatal)', e.message); }

  // Log both every run so the rolling window can be calibrated against the live
  // calendar-day behaviour before it's ever trusted to fire an alert.
  const summ = (list) => (list && list.length ? list.map((s) => `${s.brand} ${s.today}/${r1(s.mean)}+${r1(K)}σ ${s.multiple}×`).join(', ') : 'none');
  const useRolling = process.env.SURGE_ROLLING === '1';
  console.log(`surge detect · calendar-day:[${summ(dayS)}] · rolling-24h:[${summ(rollS)}]${useRolling ? ' · rolling ACTIVE' : ''}`);

  // Default returns the calendar-day surges (unchanged). SURGE_ROLLING=1 promotes
  // the rolling window; if it failed this run, fall back to calendar (never miss).
  const rollingActive = useRolling && Array.isArray(rollS);
  const chosen = rollingActive ? rollS : dayS;
  const chosenBucketOf = rollingActive ? rollBucketOf : dayOf;
  const chosenCurrent = rollingActive ? 0 : today;
  if (!chosen.length) return [];

  // Attach the current window's top contributing stories (by spread) for the body.
  const curRows = await recentItems({ days: 1 }).catch(() => []);
  const byId = new Map((curRows || []).map((r) => [r.id, r]));
  for (const s of chosen) {
    s.topStories = negatives
      .filter((n) => chosenBucketOf(n) === chosenCurrent && brandOf(n) === s.brand)
