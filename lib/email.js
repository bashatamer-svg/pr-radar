...
/* ============================================================
   Instant single-item alert (Impact 4-5, or any negative Vodafone story)
   ============================================================ */

/**
 * Does this story warrant an alert the moment it lands, rather than waiting for
 * the 05:00 brief? Impact 4-5, or ANY negative Vodafone story regardless of
...
export function isInstantAlert(item) {
  return (item.importance || 0) >= 4
    || (item.brand === 'Vodafone' && item.sentiment === 'negative');
}
...
export function urgentTier(item) {
  ...
  return { label: 'ALERT', why: 'a negative Vodafone story' };
}
...
<table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:${PAPER};">
  ...
  <tr><td bgcolor="${PAPER_2}" style="background:${PAPER_2};padding:18px 22px 24px;font-family:${FONT};" class="px">
    <div style="font-size:11px;line-height:1.6;color:${MUTED_2};">
      Instant alerts fire for fresh stories at Impact 4-5, and for any negative Vodafone story — checked every 15 minutes, silent otherwise.<br>
...
export async function sendOpsAlert(subject, lines = [], opts = {}) {
  ...
  try {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: process.env.RADAR_FROM, to, subject: String(subject).slice(0, 180), html, text }),
    });
    if (!res.ok) { console.error('ops alert resend', res.status, (await res.text()).slice(0, 160)); return false; }
    return true;
  } catch (e) {
