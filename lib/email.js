...
export function itemCardHtml(it, boardBase) {
  ...
  const byline = `<span dir="auto" style="color:${it.author ? INK_SOFT : MUTED_2};">${it.author ? 'By ' + esc(it.author) : 'Author —'}</span>`;
...
<table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:${PAPER};">
  ...
  <tr><td style="padding:24px 22px 8px;" class="px">
    ...
    <div dir="auto" style="font-family:${FONT};font-size:12px;color:${item.author ? INK_SOFT : MUTED_2};padding-top:8px;">${item.author ? 'By ' + esc(item.author) : 'Author —'}</div>
