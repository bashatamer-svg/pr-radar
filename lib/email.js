  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:10px 0 4px;background:${PAPER_2};border-radius:8px;">
    <tr><td style="padding:8px 12px;">
      <table role="presentation" cellpadding="0" cellspacing="0"><tr>
        <td style="padding:2px 10px 2px 0;font-family:${FONT};font-size:9px;font-weight:bold;color:${OXBLOOD};letter-spacing:1.5px;text-transform:uppercase;white-space:nowrap;">Impact</td>
        ${items}
      </tr></table>
    </td></tr>
  </table>`;
...
function renderAngleEmail(text, valColor, valFontSize = 13) {
  ...
  if (!lines.length) {
    return `<div dir="auto" style="font-size:${valFontSize}px;line-height:1.45;color:${valColor};padding-top:3px;">${esc(src)}</div>`;
  }
  const row = (label, val) => `<tr>
      ...
      <td valign="top" dir="auto" style="padding:4px 0;font-size:${valFontSize}px;line-height:1.45;color:${valColor};">${esc(val)}</td>
    </tr>`;
...
function coverageHtml(insts) {
  ...
  return `
  <div style="margin-top:12px;padding:10px 13px;background:${PAPER_2};border:1px solid ${HAIRLINE};border-radius:11px;font-family:${FONT};">
    <div style="font-size:9.5px;font-weight:bold;letter-spacing:.1em;color:${MUTED};text-transform:uppercase;padding-bottom:5px;">Coverage · ${list.length} outlet${list.length === 1 ? '' : 's'}</div>
    ${rows}${more}
  </div>`;
}
...
function bylineText(it) {
  if (it.author) return `By ${esc(it.author)}`;
  const desk = (Array.isArray(it._instances) && it._instances[0] && it._instances[0].outlet) || it.source || '';
  return desk ? `${esc(desk)} · newsroom` : 'Author —';
}
...
export function itemCardHtml(it, boardBase) {
  ...
  const brandChip = it.brand
    ...
    : '';
  // The board renders the category as plain uppercase micro-text, not a chip.
  const catChip = it.category
    ? `<span style="color:${MUTED};font-size:9.5px;letter-spacing:.03em;text-transform:uppercase;">${esc(String(it.category).replace(/_/g, ' '))}</span>`
    : '';
  const sentBadge = `<span style="display:inline-block;font-size:9px;font-weight:bold;letter-spacing:1px;text-transform:uppercase;padding:2px 7px;border-radius:4px;${s.pill};">${esc(sentLabel(it))}</span>`;
  const byline = `<span dir="auto" style="color:${it.author ? INK_SOFT : MUTED_2};">${bylineText(it)}</span>`;
  const boardItem = it.id ? `${boardBase}#item-${it.id}` : boardBase;
  const headlineHref = it.id ? `${ORIGIN}/api/go?id=${it.id}` : (it.url || BOARD_LINK);
  // Card chrome mirrors the board's .card: white, hairline border, rounded, no
  // left spine — the sentiment reads from the border tint + the pill instead.
  // (Outlook squares the corners; everything else matches the app.)
  return `
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:11px 0;"><tr>
    <td bgcolor="${CARD}" style="background:${CARD};border:1px solid ${s.line};border-radius:14px;padding:15px 16px;font-family:${FONT};">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
        <td style="font-size:11px;color:${MUTED_2};padding-right:12px;">
          ${brandChip}${catChip ? '&nbsp;' + catChip : ''}
          &nbsp;<span style="color:${FAINT};">${outletAge}</span>
        </td>
        <td align="right" valign="top" style="font-size:11px;line-height:1;padding-top:2px;white-space:nowrap;">
          <span style="font-family:${FONT};font-size:9px;font-weight:bold;color:${FAINT};letter-spacing:.05em;text-transform:uppercase;margin-right:6px;">Impact</span>${dots}
        </td>
      </tr></table>
      <div dir="auto" style="font-size:17px;font-weight:bold;line-height:1.32;letter-spacing:-.02em;color:${INK};padding-top:10px;">
        <a href="${esc(headlineHref)}" style="color:${INK};text-decoration:none;">${esc(it.headline)}</a>
      </div>
      <div style="font-size:12px;padding-top:9px;">${sentBadge} &nbsp; ${byline}</div>
      ${it.summary ? `<div dir="auto" style="font-size:14px;line-height:1.58;color:${INK_SOFT};padding-top:9px;">${esc(it.summary)}</div>` : ''}
      ${it.pr_angle
        ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:12px;"><tr>
             <td bgcolor="${s.bg}" style="background:${s.bg};border:1px solid ${s.line};border-radius:12px;padding:11px 13px;">
               <div style="font-size:9.5px;font-weight:bold;letter-spacing:.1em;color:${MUTED};text-transform:uppercase;padding-bottom:2px;">What to do with this</div>
...
const SENT_SECTIONS = [
  { key: 'negative', label: 'Needs a response', color: RED,       flag: '&#9888; ' },
  { key: 'positive', label: 'Wins to amplify',  color: '#0f9d58', flag: '' },
  { key: 'neutral',  label: 'Market & noted',   color: '#5b6675', flag: '' },
];
...
<table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:${PAPER};">
  ...
  <tr><td bgcolor="#fbf0d9" style="background:#fbf0d9;border-bottom:1px solid #ecd9a6;padding:9px 22px;font-family:${FONT};" class="px">
    <div style="font-size:12px;line-height:1.5;color:#6b5836;"><strong style="color:#8a5a00;">Private — please don't forward.</strong> This brief contains non-public brand &amp; reputation analysis.</div>
  </td></tr>
...
export function urgentTier(item) {
  ...
  if (imp >= 5) return { label: 'URGENT', why: 'a severity-5 reputational threat' };
  if (imp >= 4) return { label: 'ALERT', why: 'an Impact-4 story' };
  // Name the brand: since rivals' negatives alert too, "a negative story" alone
  // no longer tells the reader whether this is our problem or a competitor's —
  // which is the first thing they need to know from the header.
  return { label: 'ALERT', why: `a negative ${item.brand || 'brand'} story` };
}
...
<body style="margin:0;padding:0;background:${CANVAS};">
<div style="display:none;max-height:0;overflow:hidden;mso-hide:all;font-size:1px;line-height:1px;color:${CANVAS};">
  ${tier.label} · ${esc(item.brand || 'Brand')} — ${esc((item.headline || '').slice(0, 140))}&nbsp;&zwnj;
</div>
...
<table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:${PAPER};">
  ...
  <tr><td bgcolor="${RED}" style="background:${RED};padding:16px 22px 14px;border-top:5px solid ${OXBLOOD};" class="px">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
      ...
      <td align="right" valign="middle" style="font-family:${FONT};">
        <span style="display:inline-block;background:${OXBLOOD};color:#ffffff;font-size:13px;font-weight:bold;letter-spacing:3px;padding:7px 14px;border-radius:6px;">● ${tier.label}</span>
      </td>
    </tr></table>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:13px;"><tr>
      <td style="border-top:3px solid #ffffff;padding-top:11px;font-family:${FONT};font-size:11px;line-height:1.5;color:#ffc9c9;letter-spacing:.4px;">
        Flagged within 15 minutes — <strong style="color:#ffffff;">${tier.why}</strong> · ${timeCairo} Cairo
      </td>
    </tr></table>
  </td></tr>
  ...
  <tr><td style="padding:24px 22px 8px;" class="px">
    <table role="presentation" cellpadding="0" cellspacing="0"><tr>
      <td style="font-family:${FONT};font-size:11px;color:${MUTED_2};">
        ...
        <span style="padding-left:8px;color:${s.color};font-weight:bold;text-transform:uppercase;">${esc(sentLabel(item))}</span>
      </td>
    </tr></table>
    ...
    <div dir="auto" style="font-family:${FONT};font-size:12px;color:${item.author ? INK_SOFT : MUTED_2};padding-top:8px;">${bylineText(item)}</div>
  ...
  <tr><td bgcolor="${PAPER_2}" style="background:${PAPER_2};padding:18px 22px 24px;font-family:${FONT};" class="px">
    <div style="font-size:11px;line-height:1.6;color:${MUTED_2};">
      Instant alerts fire for fresh stories at Impact 4-5, and for any negative story about Vodafone, Orange, WE or e&amp; — checked every 15 minutes, silent otherwise.<br>
...
/** The per-person sign-in email. Deliberately PERSONAL: each recipient sees only
    their OWN address and password. The starter password follows a guessable
    convention (first name + 123), so a list of them in one thread would hand
    every reader everyone else's key — which is also why this is never sent to a
    group and never re-sent to someone who already has a password.
  ...
  <tr><td style="padding:0 26px;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
      ...
      <div style="font-family:${FONT};font-size:14px;line-height:1.6;color:${INK_SOFT};">
        <b>Change that password.</b> It was generated from your first name, so it
        is deliberately temporary. Once you are in, open <b>Password</b> in the
        top bar of the board. It takes a few seconds and you only do it once.
      </div>
    </td></tr></table>
  </td></tr>
 ...
 *
 *  Recipients: OPS_ALERT_TO, else RADAR_TO (the team brief list), else the
 *  active subscribers are NOT used — an ops alert is for whoever runs the
 *  radar, not for its readers. */
export async function sendOpsAlert(subject, lines = [], opts = {}) {
  ...
  try {
    const to = parseAddrs(process.env.OPS_ALERT_TO || process.env.RADAR_TO);
