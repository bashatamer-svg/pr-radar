...
// Tone is ALWAYS judged from Vodafone's standpoint, so a rival's good news is
// stored as "negative" (it moves the market unfavourable). A bare "Negative" pill
// on a competitor card reads as a mislabel, so those pills say what the story IS
// while the colour keeps saying what it MEANS for us. Display-only — the stored
// sentiment, lanes and analytics are untouched. Mirrors the board's pill.
const COMP_LABEL = { negative: 'Competitor win', positive: 'Competitor setback', neutral: 'Competitor note' };
const isCompetitor = (it) => ['Orange', 'WE', 'e&'].includes(it && it.brand);
const sentLabel = (it) => (isCompetitor(it)
  ? COMP_LABEL[it && SENT[it.sentiment] ? it.sentiment : 'neutral']
  : sentOf(it).label);
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:10px 0 4px;background:${PAPER_2};border-radius:8px;">
    <tr><td style="padding:8px 12px;">
      <table role="presentation" cellpadding="0" cellspacing="0"><tr>
        <td style="padding:2px 10px 2px 0;font-family:${FONT};font-size:9px;font-weight:bold;color:${OXBLOOD};letter-spacing:1.5px;text-transform:uppercase;white-space:nowrap;">Reach</td>
        ${items}
      </tr></table>
    </td></tr>
  </table>`;
    ...
    <td bgcolor="${CARD}" style="background:${CARD};border:1px solid ${s.line};border-radius:14px;padding:15px 16px;font-family:${FONT};">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
        ...
        <td align="right" valign="top" style="font-size:11px;line-height:1;padding-top:2px;white-space:nowrap;">
          <span style="font-family:${FONT};font-size:9px;font-weight:bold;color:${FAINT};letter-spacing:.05em;text-transform:uppercase;margin-right:6px;">Reach</span>${dots}
        </td>
      </tr></table>
