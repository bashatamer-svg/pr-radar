...
export const SITE_FEEDS = [
  ...
  // Operator websites are deliberately NOT swept (user decision, 5 Aug). A
  // `site:vodafone.com.eg`-style sweep of the four operators' own domains ran
  // until then and never produced news: what Google indexes on an operator
  // domain is store pages, promo pages and personalised share cards — one of
  // which put a customer's private mobile number into the daily brief. Owned
  // announcements that matter reach the radar through the press that covers
  // them, which is the signal a PR team actually wants.
