...
// The brand-targeted subset — the four operators' own Google News queries (the
// entries carrying a `brand`), excluding the market sweep (brand:null) and the
// broad DIRECT_FEEDS outlet firehoses. Used by the near-real-time urgent-only
// poll (api/radar.js?urgentOnly=1): a tight, fast net over exactly the four
// brands, cheap enough to run every 15–30 min. The daily full run still sweeps
// ALL_FEEDS. NOTE: this deliberately omits the market sweep (the "outage/
// complaint" cross-operator query) — flip a story into BRAND_FEEDS by giving its
// feed a `brand`, or widen this to `FEEDS` if the crisis net should include it.
export const BRAND_FEEDS = FEEDS.filter((f) => f.brand);
