...
// only — the 30-min urgent poll keeps to the tight brand queries.
// ---------------------------------------------------------------------------
...
export const SITE_FEEDS = [
  ...
  { id: 'site-daily-ar2', tier: 1, brand: null, country: 'Egypt',
    url: gnews(`${sites('elbalad.news', 'mobtada.com', 'dostor.org', 'shorouknews.com', 'gate.ahram.org.eg')} ${AR_BRANDS}`, 'ar') },
...
// Mark the outlet firehoses so the pipeline can prefilter them (see below).
// SITE_FEEDS are query-scoped like FEEDS, so they are NOT prefiltered.
...
// net cheap enough to run every 15–30 min. The market sweep is now INCLUDED so a
