...
// Tag / keyword / search / category ARCHIVE pages are site navigation, not
// articles. Google News sometimes serves them as "stories" (besraha.com
// /keyword/318029, elghad.news/tag/…) — and a tag page has no real headline,
// no byline (its "author" scrapes as search-box placeholder text) and no event,
// so everything downstream fabricates. Matched by whole path SEGMENT so a
// WordPress single-post permalink like /archives/341270 is NOT caught.
export function isNonArticlePage(u) {
  try {
    const { pathname, searchParams } = new URL(u);
    if (/(^|\/)(tag|tags|keyword|keywords|category|categories|topic|topics|search)(\/|$)/i.test(pathname)) return true;
    for (const k of ['s', 'q', 'search']) if (searchParams.has(k)) return true;
    // Personalised share cards. Google indexed a Vodafone Cash year-in-review
    // page (web.vodafone.com.eg/cash/share?…&a=27&b=2010…), the site: newsroom
    // sweep swallowed it, and the daily brief mailed a private mobile number to
    // the whole list as a "win" (live, 5 Aug). Two independent tells, either
    // sufficient: a /share path segment (a share endpoint is a thing you are
    // handed, not a thing a newsroom wrote), and a full phone number as a query
    // VALUE — no article URL carries one, every personalised card does.
    if (/(^|\/)share(\/|$)/i.test(pathname)) return true;
    for (const v of searchParams.values()) if (/^\+?\d{11,15}$/.test(v)) return true;
    return false;
  } catch { return false; }
}
