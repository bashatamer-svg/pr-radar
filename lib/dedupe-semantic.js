...
- the same DISPUTE, accusation, complaint, lawsuit or public controversy, however the parties are described — an accusation reported by two outlets is one event, not two.
...
NAMES ARE SPELLED MANY WAYS. These summaries are paraphrases of Arabic coverage, and the same person, artist, programme or title is transliterated differently every time: Walid / Waleed, Mohamed / Mohammed / Muhammad, Tawlet / Too Late, Hiya Taqod / She Leads. Two spellings that plausibly render the SAME Arabic name are the same party — never call two items different events merely because a name is spelled differently, or because one item names a party the other only describes ("another artist"). Judge the EVENT: same act, same subject, same brand, same time.
...
async function sameEventBatch(pairs) {
  // Both summaries are classifier paraphrases OF third-party coverage, so they
  // carry whatever the source wrote. Neutralised per field, fenced as a block.
  const list = pairs
    .map((p, i) => `${i}. EXISTING: ${neutralize(p.existing, { max: 400 })}\n   NEW: ${neutralize(p.item.summary, { max: 400 })}`)
    .join('\n');
  const system = SAME_EVENT_SYSTEM;
  // Headlines and summaries are third-party text — fenced, and neutralised so
  // neither can close the fence. See lib/prompt-safety.js.
  const userContent = fenceUntrusted(list);
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: MODEL, max_tokens: 700, system, messages: [{ role: 'user', content: userContent }] }),
  });
  if (!res.ok) throw new Error(`anthropic ${res.status}`);
  const data = await res.json();
  recordUsage('dedupe', MODEL, data);   // fire-and-forget spend gauge; never awaited
  const text = data.content.filter((b) => b.type === 'text').map((b) => b.text).join('').replace(/```json|```/g, '').trim();
  try { return JSON.parse(text); } catch { /* salvage the [ … ] span below */ }
...
// recentSummaries: recently-stored items as {id, summary} (bare strings still
//   work — they simply carry no id to merge into).
// opts.onMerge(item, intoId): called for each confirmed repost, so the caller
//   can attach the dropped item's outlet to the card it duplicates instead of
//   losing that coverage. intoId is null for a within-run match (the twin has
//   no DB id yet; the lexical pass already merged those as instances).
// Returns the items to KEEP (same array minus confirmed same-event reposts).
export async function semanticDedupe(items, recentSummaries = [], opts = {}) {
  ...
  const pool = recentSummaries.filter(Boolean).map((r) => {
    const s = typeof r === 'string' ? r : r.summary;
    const t = tok(s);
    return { id: typeof r === 'string' ? null : r.id, summary: s, tokens: t, strong: strongToks(t), bi: bigrams(s) };
  });
  const pairs = [];
  for (const it of list) {
    ...
    if (best && (bestJ >= LOW || bestSh >= STRONG_MIN || bestBg >= BIGRAM_MIN)) {
      pairs.push({ item: it, existing: best.summary, existingId: best.id ?? null, score: bestScore });
    }
    // A later item this run can match an earlier one: no DB id yet, so it has
    // no merge target — the lexical within-run pass owns those.
    pool.push({ id: null, summary: it.summary, tokens: t, strong: ts, bi: tb });
  }
  ...
  try {
    ...
    for (const v of verdicts) {
      if (!v || !v.same || !pairs[v.i]) continue;
      const { item, existingId } = pairs[v.i];
      dropHashes.add(item.hash);
      // Hand the caller the card this repeats, so the outlet is kept as coverage.
      if (typeof opts.onMerge === 'function') {
        try { opts.onMerge(item, existingId); } catch { /* never let a callback kill dedupe */ }
      }
    }
    if (dropHashes.size) console.log(`semantic dedupe: merged ${dropHashes.size} same-event repost(s) of ${pairs.length} borderline pair(s)`);
