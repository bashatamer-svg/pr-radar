...
//
// PROBED 3 Aug from production: 1 of 31 worked (Aitnews — now live in
// DIRECT_FEEDS). Most of the rest 404 or 403: these were guessed URL shapes
// (/rss, /feed) and Egyptian sites overwhelmingly do not use them.
//
// SIX answered 200/202 with no items, which is a different failure and the only
// group worth more effort — the host is alive and serving something, so the URL
// shape is wrong rather than the outlet being feedless: Amwal Al Ghad (AR),
// Asharq Business (202), Al Shorouk, Youm7 (first URL only), Orange Egypt
// newsroom, MCIT. The probe now reports WHAT came back for these (content type
// + whether it was an HTML page), so the next pass can chase a real path
// instead of guessing another suffix.
export const FEED_CANDIDATES = [
  ...
  // ---- Telecom / tech trade ----
  // (Aitnews promoted to DIRECT_FEEDS 3 Aug — verified, 10/10 bylined.)
