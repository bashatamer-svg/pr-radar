...
export const FEED_CANDIDATES = [
  ...
  // `site:youlyou.com` in site-daily-ar2, so its brand coverage is landing
  // either way; a direct feed would only add the byline. These three URLs are
  // GUESSES, staged rather than promoted for exactly that reason — the site
  // organises by /channel/<n>, so a channel-scoped feed is the likeliest shape
  // if it publishes one at all. The probe's reason line is what makes guessing
  // here cheap: it will say whether a feed exists at a different path (HTML
  // page) or not at all (404), instead of leaving another dead row behind.
  { id: 'youlyou', name: 'Youlyou', kind: 'general',
    urls: ['https://www.youlyou.com/rss', 'https://www.youlyou.com/feed', 'https://www.youlyou.com/rss/1'] },
