...
async function splitRetry(chunk, extraSystem, reask, why) {
  const mid = Math.ceil(chunk.length / 2);
  console.warn(`classify batch of ${chunk.length} failed (${why}); splitting ${mid}/${chunk.length - mid} and retrying`);
  const [a, b] = await Promise.all([
    classifyChunk(chunk.slice(0, mid), extraSystem, reask),
    classifyChunk(chunk.slice(mid), extraSystem, reask),
  ]);
  return [...a, ...b];
}

async function classifyChunk(chunk, extraSystem, reask = 0) {
  let out;
  try {
    ...
    return splitRetry(chunk, extraSystem, reask, e.message);
  }
  ...
  if (missing.length === chunk.length && chunk.length > 1) {
    ...
    return splitRetry(chunk, extraSystem, reask + 1, 'model returned no verdicts');
  }
  console.warn(`classify: model skipped ${missing.length}/${chunk.length} item(s); re-asking for just those`);
  const redo = await classifyChunk(missing.map((i) => chunk[i]), extraSystem, reask + 1);
