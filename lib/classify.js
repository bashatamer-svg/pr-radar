...
or a mobile-market business trend a comms team should know. Judge sentiment from
Vodafone's standpoint (a market price-rise the public resents is negative for the
sector, Vodafone included) and severity by reach. This does NOT reopen the
...
2. sentiment — the reputational direction FROM VODAFONE EGYPT'S STANDPOINT:
   ...
   COMPETITOR ITEMS — run this test EVERY time; it is the most common
   mis-scoring. Ask "does this make the COMPETITOR look better or worse?",
   then INVERT the answer for Vodafone:
     - competitor looks BETTER → "negative" (revenue/subscriber growth, an
       award, ranking first on any measure, a praised product or campaign, a
       CSR or diversity programme, an executive honoured, a new partnership);
     - competitor looks WORSE → "positive" (outage, fraud or service
       complaints, a regulatory penalty, a viral pile-on).
   NEVER score a competitor item by how the story feels for THAT company —
   this field always means the direction for Vodafone Egypt.
   Worked examples: "e& revenue up 21%" → negative. "WE ranks first in search
   interest" → negative. "Orange leads a national digitisation project" →
   negative. "Orange launches a women-leadership programme" → negative.
   "Public figure accuses Orange of fraud" → positive.

   SECTOR SPILLOVER (overrides the inversion): if a competitor's problem is
   really an industry-wide issue (a sector-wide outage, a data-privacy scandal
   that implicates all operators, a price row the public blames on "the mobile
   companies"), it is NEGATIVE for Vodafone too, because the public tars the
   whole sector. Say so in the angle.
