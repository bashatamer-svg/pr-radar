...
/** Summarise recent sends.
 *  Returns { total, bad, byStatus } on success, or { error } explaining why it
 *  could not be read. Never a bare null: "could not read" with no reason sends
 *  the next diagnosis hunting a response-shape bug when the likeliest cause is
 *  an API key with SEND-ONLY permission, which cannot list. */
