...
function fromMeta(html) {
  // Ordered by reliability. article:author is sometimes a URL, not a name —
  // cleanName rejects the long/URL cases.
  const patterns = [
    /<meta[^>]+name=["']author["'][^>]+content=["']([^"']+)["']/i,
    /<meta[^>]+property=["']article:author["'][^>]+content=["']([^"']+)["']/i,
    /<meta[^>]+name=["']sailthru\.author["'][^>]+content=["']([^"']+)["']/i,
  ...
  for (const re of patterns) {
    ...
    if (m) {
      ...
      if (c && !/^https?:\/\//i.test(m[1])) return c;
    }
  }
  return null;
}
