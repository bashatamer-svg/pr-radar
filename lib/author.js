...
export function extractAuthorFromHtml(html, outlet = null) {
  if (!html) return null;
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system:
          ...
          'mentioned or quoted IN the story (an official, a CEO). If no byline is written, return null. ' +
          'Do not guess.',
...
export async function fetchAuthorProbe(url, outlet = null) {
  ...
  try {
    ...
    const author = extractAuthorFromHtml(html, outlet) || await aiExtractAuthor(htmlToText(html));
...
export async function fetchAuthor(url, outlet = null) {
  return (await fetchAuthorProbe(url, outlet)).author;
}
...
export async function inspectAuthorPage(url, outlet = null) {
  const empty = { author: null, textStart: '', candidates: [] };
  ...
  if (!res || !res.ok) return { ...empty, outcome: 'fetch-failed', status: res ? res.status : null };
  try {
    const html = await res.text();
    const candidates = [...jsonLdCandidates(html), ...metaCandidates(html), ...bylineCandidates(html)]
      .slice(0, 6).map((c) => String(c).slice(0, 60));
    const author = extractAuthorFromHtml(html, outlet) || await aiExtractAuthor(htmlToText(html));
    return { outcome: author ? 'found' : 'no-byline', status: res.status, author, textStart: htmlToText(html).slice(0, 300), candidates };
  } catch { return { ...empty, outcome: 'fetch-failed', status: null }; }
}
