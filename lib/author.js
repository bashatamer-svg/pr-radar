...
export function extractAuthorFromHtml(html, outlet = null, { headline = null } = {}) {
  ...
  // The page's own reading window, so the subject check sees what the extractor
  // saw. Computed once, and only when there is something to check.
  let text = null;
  for (const c of candidates) {
    ...
    if (text === null) text = htmlToText(html);
