...
// Dateline words. A byline sits next to a DATE, and every extractor that walks
// back from one can overshoot into it — `datelineCandidates` stops at an
// ALL-CAPS token or a role word, and Arabic copy has neither, so it ran back
// through a headline and kept the weekday it started from ("… السبت", live
// 8 Aug). No person's name contains a weekday in either language, so this is
// safe to apply with no context at all, which is what a bare <dc:creator>
// arrives with. Months are here for the same reason.
const DATE_WORD = new RegExp(
  '(^|\\s)(' +
  // Arabic weekdays + the two month namings Egyptian press uses
...
export const BYLINE_RULES = [
  ...
  ['dateline-word', (n) => DATE_WORD.test(n)],
  ['too-long',  (n) => n.length > 80],
];


/** The bare-string face of judgeByline: everything judgeable with NO PAGE.
    An RSS <dc:creator> or the model's answer arrives with no article text and
    no headline around it, so the story-context rules cannot run — but the
    shape and segment rules can, and until 8 Aug this skipped them.

    That was the funnel's own principle broken from the inside: the SAME credit
    got two different wrong answers depending on which door it came through.
    "Rana Mamdouh, Sara Seif Eddin" off Mada Masr's RSS was stored JOINED (this
    function returned the whole string unsplit, so the leaderboard grew a
    journalist nobody is — and the same three writers in a different order
    became a second one); the identical string reaching judgeByline from the
    HTML cascade was REJECTED outright as `not-name-shaped`. One funnel means
    one answer, so this now delegates rather than reimplementing the cheap half.

    The outlet is deliberately still not passed: a caller holding a bare string
    has one only by coincidence, and isOutletName's publication markers work
    without it. */
function cleanNameChecked(raw) {
  const v = judgeByline(raw);
...
export function judgeByline(raw, { outlet = null, headline = null, text = null } = {}) {
  ...
  // With several people ("Rana Mamdouh, Sara Seif Eddin") that means the LEAD
  // byline, and the alternative is worse than it looks: `author` is one text
  // column, so keeping the joined string makes the journalist leaderboard count
  // a person nobody is — and re-orderings of the same team become separate
  // entries (live: the same three Mada Masr writers stored two ways). Crediting
  // the lead under-reports the others; it never invents anyone, which is the
...
// colon, BOTH commas (ASCII and Arabic «،»), and ' - ' (spaces around the dash,
// so hyphenated names survive).
//
// The comma is what joins CO-AUTHORS, and leaving it out cost us real ones.
// Mada Masr credits its investigations to two to five writers, and without a
// comma here the whole credit is ONE segment, so NAME_SHAPE rejects it as
// `not-name-shaped` and the card reads newsroom. Silent by construction — the
// board shows an honest blank either way, so nobody reports it; it was found by
// sweeping pr_items, where six such rows survive from before the funnel put
// NAME_SHAPE on the single-segment path.
const SEP = /\s*[|/·•:：,،]\s*|\s+[-–—]\s+/;
