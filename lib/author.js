...
function cleanName(raw) {
  ...
  if (MARKUP_JUNK.test(s)) return null; // scraped tag residue, not a person
  if (looksLikeSlug(s)) return null;    // a URL path segment, not a person
...
export function cleanAuthor(raw, outlet) {
  ...
  // A candidate with nothing to split was previously returned UNCHECKED — the
  // person-shape test below only ever ran on multi-segment names, so a lone blob
  // of scraped markup became a byline as long as it wasn't the outlet's name.
  // That is how the Astro attribute residue reached the board.
  if (segments.length <= 1) return (isOutletName(c, outlet) || !NAME_SHAPE.test(c)) ? null : c;
