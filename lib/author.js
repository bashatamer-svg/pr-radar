...
// Fetch a RESOLVED publisher URL and extract the author: deterministic cascade
// first, AI fallback when it finds nothing. Returns null on any failure
// (timeout, 403, no byline anywhere). Never throws.
export async function fetchAuthor(url) {
  ...
  try {
    ...
    return extractAuthorFromHtml(html) || await aiExtractAuthor(htmlToText(html));
  } catch {
    return null;
  } finally {
    clearTimeout(t);
